...
Raster_GDP_Proxy_Country <- LitPop.Get_GDP_Proxy_Raster(DP_Country_ISO3, DP_Year) / 10 ^ 6
...
# Impacted States
VT_States_Core <- c("Aceh", "North Sumatra Province", "West Sumatra", 
                    "Riau Province", "Bengkulu Province", 
                    "Jambi Province", "South Sumatra", "Lampung Province")
...
text(x=MX_Coordinates[,1],
     ...
     cex = 0.55, pos = 3)
# Plot Impacted Area
CC_Impact <- circles(DF_Points, DP_Radius, lonlat = TRUE, dissolve = FALSE) 
